...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    expect(createMenu().order("coxinha")).toEqual(createMenu().consumption); 
    createMenu().cleanUpArray();     
    ...
    expect(multiTests()).toEqual(["coxinha", "agua", "sopa", "sashimi"]);
    createMenu().cleanUpArray(); 
    ...
    expect(multiTestsTwo()).toEqual(['coxinha', 'agua', 'coxinha']);
    createMenu().cleanUpArray();  
    ...
    // ```
    const multiTestsThree = () => {   
      createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} }).fetchMenu();
      createMenu().order("coxinha");
      createMenu().order("agua");
      createMenu().order("coxinha");      
      return createMenu().pay();
    }
    expect(multiTestsThree()).toBeCloseTo(11.70);
    createMenu().cleanUpArray(); 
  
    // Agora faça o PASSO 4 no arquivo `src/restaurant.js`.

    const multiTestsFour = () => {   
      createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} }).fetchMenu();
      createMenu().order("coxinha");
      createMenu().order("agua");
      createMenu().order("coxinha");      
      return createMenu().pay() * 1.1;
    }
    expect(multiTestsFour()).toBeCloseTo(12.87);
    createMenu().cleanUpArray(); 
  });
});
