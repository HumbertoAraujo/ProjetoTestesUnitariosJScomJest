...
const orderFromMenu = (receivedString) => {
  newArray.push(receivedString);
  return newArray;
};
...
const closePay = () => {
  let sum = 0;
  for (let arrayItem of newArray) {
    if (menuObject.food[arrayItem] !== undefined) {      
      sum += menuObject.food[arrayItem];
    }
    if (menuObject.drink[arrayItem] !== undefined) {      
      sum += menuObject.drink[arrayItem];
    }
  }
  return sum;  
};
...
const createMenu = (receivedObject) => {
  newObject.fetchMenu = () => { menuObject = receivedObject; return menuObject; };
  newObject.consumption = newArray;
  newObject.order = orderFromMenu;
  newObject.cleanUpArray = cleanUp;
  newObject.pay = closePay;
  return newObject;
};
